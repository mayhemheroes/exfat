...
## Run Information
- Mayhem Run: mayhemheroes/exfat/exfat-debug/5
- Git Commit: dbdce42081dbc5f94a9c0b75d01a5b2841a5a081
- Original Image: ghcr.io/mayhemheroes/exfat:master
- Target Executable: /fuzz_exfat_debug
- CI URL: https://github.com:443/mayhemheroes/exfat//actions/runs/4640305610
...
- Source code: `/rlenv/source/exfat/`
...
RUN cp <build-artifact> /fuzz_exfat_debug

# 11. Permission Management (REQUIRED)
RUN chmod 766 /fuzz_exfat_debug
...
- `COPY . /rlenv/source/exfat/` happens early (source exists)
...
- ✅ Must work from `/rlenv/source/exfat/`

## Critical Requirements Checklist

### Permissions (REQUIRED)
- [ ] Source directory: `chmod -R 777 /rlenv/source`
- [ ] Target executable: `chmod 766 /fuzz_exfat_debug`
...
- [ ] Target executable in Dockerfile.rlenv: `/fuzz_exfat_debug`
- [ ] Target executable in build.sh: `/fuzz_exfat_debug`
- [ ] Target executable in metadata.json: `/fuzz_exfat_debug`
- [ ] All must match for validation to work

### COPY Order (REQUIRED)
- [ ] `COPY . /rlenv/source/exfat/` - EARLY (source exists now)
...
WORKDIR /rlenv/source/exfat/fuzz
...
- Source copy: `COPY . /rlenv/source/exfat/`
...
- Working directory: `cd /rlenv/source/exfat/`
- No network access
